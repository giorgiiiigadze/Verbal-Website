    ...
    <Section id="how">
      ...
      */}
      <div className="grid items-center justify-items-center gap-16 lg:grid-cols-[320px_minmax(0,36rem)] lg:justify-center lg:gap-24">
        ...
        <div>
          {/* The eyebrow is set by hand rather than handed to SectionHeading,
              which would style it as every other section's — this one copies
              the social-proof band above it: bold, capitalised, wide-tracked
              and muted, so the two labels read as the same device. */}
          <p
            data-proof-reveal
            className="text-[16px] font-semibold capitalize text-[#8D8D8D]"
          >
            How it works
          </p>
          <SectionHeading
          ...
          <ol className="mt-12 space-y-10">
            {STEPS.map((step) => (
              <li key={step.number} className="flex gap-5">
                <span
                  aria-hidden="true"
                  className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-line bg-card font-slab text-sm text-accent-text"
                >
                  {step.number}
                </span>
                <div>
